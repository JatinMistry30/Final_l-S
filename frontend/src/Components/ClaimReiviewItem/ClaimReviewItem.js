...
const ClaimReviewItem = () => {
  ...
  useEffect(() => {
    const fetchReportDetails = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/api/notification/claim-reports/${id}`, {
  ...
  const handleResponse = async (status) => {
    try {
      await axios.post(
        `http://localhost:5000/api/notification/claim-reports/${id}/respond`,
        { status },
        { withCredentials: true }
      );
      ...
      <div className="card-container-review-found">
        <div className="grid-container-review-found">
          ...
          <div className="finder-details-section-review-found">
            <h2 className="section-title-review-found">Claimer`s Details</h2>
            <p className="section-content-review-found">
              <span className="content-label-review-found">Claimed By:</span> {report.finderEmail}
            </p>
            <p className="section-content-review-found">
